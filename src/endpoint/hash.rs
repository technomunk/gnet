...
pub fn read_hash(packet: &[u8]) -> u64 {
	debug_assert!(packet.len() > size_of::<u64>());
	debug_assert!(packet.as_ptr().align_offset(std::mem::align_of::<u64>()) == 0);
	#[allow(clippy::cast_ptr_alignment)]
	unsafe {
		*(packet.as_ptr() as *const u64)
	}
}

/// Generate the hash for the provided packet data.
pub fn generate_hash<H: Hasher>(packet: &[u8], mut hasher: H) -> u64 {
	debug_assert!(packet.len() > size_of::<u64>());
	hasher.write(&packet[size_of::<u64>()..]);
	hasher.finish()
}
...
pub fn generate_and_write_hash<H: Hasher>(packet: &mut [u8], hasher: H) {
	debug_assert!(packet.len() > std::mem::size_of::<u64>());
	debug_assert!(packet.as_ptr().align_offset(std::mem::align_of::<u64>()) == 0);
	let hash = generate_hash(packet, hasher);
	unsafe { *(packet.as_mut_ptr() as *mut u64) = hash }
}
